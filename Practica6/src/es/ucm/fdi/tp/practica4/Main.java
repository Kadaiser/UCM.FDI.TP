...
import es.ucm.fdi.tp.practica4.FourLine.FourLineFactory;
...
public class Main {
	...
	enum GameInfo {
		...
		Ataxx("at","Ataxx"), Buscaminas("bm", "Buscaminas"), FOURLINE("4l", "FourLine");
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case Buscaminas:
			if (dimRows != null && dimCols != null) {
				if (obstacles != null)
					gameFactory = new BuscaMinasFactory(dimRows, dimCols, obstacles);
				else
					gameFactory = new BuscaMinasFactory(dimRows, dimCols);
			} else if (obstacles != null)
				gameFactory = new BuscaMinasFactory(7, 7, obstacles);
			else
				gameFactory = new BuscaMinasFactory();
			break;
		case FOURLINE:
			if (dimRows != null && dimCols != null) {
					gameFactory = new FourLineFactory(dimRows, dimCols);
			} 
			else if (dimRows != null)
				gameFactory = new FourLineFactory(7, dimCols);
			
			else if (dimCols != null)
				gameFactory = new FourLineFactory(dimCols, 7);
			
			else
				gameFactory = new FourLineFactory();
			break;
		default:
